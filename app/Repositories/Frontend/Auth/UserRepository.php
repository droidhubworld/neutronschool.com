...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        ...
        $default_role = array_key_exists("role", $data) ? $data['role'] : config('access.users.default_role');

        return DB::transaction(function () use ($user, $default_role) {
            if ($user->save()) {
                //Attach new roles
                if ($roles = Role::where('name', $default_role)->get()->pluck('id')->toArray()) {
    ...
    protected function createUserStub($input)
    {
        ...
        $user->email = $input['email'];
        $user->is_term_accept = array_key_exists("is_term_accept", $input) ? $input['is_term_accept'] : 0 ;
